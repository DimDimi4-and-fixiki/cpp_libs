...
class SegmentTree  // класс дерева отрезков
{
    public:
    ...
    inline void update(int v, int left, int right, int idx, int value) // обновление в вершине
    {
        ...
        int mid = (left + right) / 2;
        if (idx <= mid) {  // спускаемся по дереву влево
            update(v * 2, left, mid, idx, value);
        }
        else {  // спускаемся по дереву вправо
    ...
    {
        if (l > right || r < left || l > r){  // вышли за отрезок
            return -1e9;
        }
        if (l == left && r == right) {  // попали в нужную вершину
            return max_tree[v];
        }
        else {  // разбиваем на два отрезка и рекурентно считаем для каждого
            int mid = (left + right) / 2;
